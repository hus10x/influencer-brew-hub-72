      ...
      {(provided, snapshot) => (
        <div
          ...
          className={`flex flex-col h-full rounded-lg ${
            snapshot.isDraggingOver ? "bg-muted/50 dark:bg-muted/10" : "bg-muted/30 dark:bg-background/95"
          }`}
          style={{
        ...
        >
          <div className="flex items-center justify-between p-4 border-b border-border/50">
            <h3 className="font-semibold capitalize text-lg text-foreground dark:text-foreground">{status}</h3>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {campaigns.length === 0 ? (
              <div className="flex items-center justify-center h-32 border-2 border-dashed border-muted rounded-lg">
                <p className="text-sm text-muted-foreground dark:text-muted-foreground/90">Drop campaigns here</p>
              </div>
            ) : (
